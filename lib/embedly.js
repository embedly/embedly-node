  ...
  emitter.start = function() {
    ...
    request.on('response', function(response) {
      ...
      response.on('end', function() {
        console.log('emitting complete with %s', data)
...
/**
 * A simple wrapper around node-syslog.  If node-syslog is not present, then
 * all logging functions are noop.
 *
 * ex::
 *   var embedly = require('embedly')
 *   embedly.log.debug('hi log!')
 */
try {
...
/*
 * Class representing the Embedly Api.  Parameters are passed as a object.
...
Api.prototype = new(function() {
   ...
   * }
   *
   * args are used to hold all other parameters, including the actual url
   * parameters and the callback funciton::
   *
   *   args: {
   *     params: {
   ...
   *     }
   *   , complete: [required] // function(error, objects) called to handle the embedly response.
   *   }
   *
   * ex::
   *   api.apicall(
    ...
    services_regex(function(e, sregex) {
      if (e) {
        ...
        if (params['urls'].length > 0) {
          ...
          this.emit('complete', rejects.map(function(i){return i[1]}))
        }
      }
    })
  }
  ...
  /**
   * Responds via `oncomplete(error, obj)` with the services object from the embedly
   * `services api <http://api.embed.ly/docs/service>`_
   *
   * ex::
   *   api.services(function(error, obj) {
   *     if (error) {
   *       console.log('an error occurred')
   *       console.log(error)
   *     } else {
   ...
   *
   * Response via `oncomplete(error, obj)` with a regex to match
   * accepted providers urls against.
   *
   * ex::
   *   urls = [...]  // some urls
   *   api.services_regex(function(error, regex) {
   *     if (error) {
   *       console.log('an error occurred')
   *       console.log(error)
   *     } else {
